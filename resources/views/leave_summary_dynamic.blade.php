...
<thead>
<tr>
    <th colspan="{{(count($leave_codes)*2)+3}}" align="left">Date Range : From  {{$startDate ?? ''}} To {{$endDate ?? ''}}</th>
</tr>
<tr>
    <th colspan="3" align="left">Name: {{$employee->first_name ?? ''}} {{$employee->last_name ?? ''}}</th>
    <th colspan="{{count($leave_codes)}}" align="left">Join Date: {{$employee->hire_date ?? ''}} </th>
...
<tr>
    ...
    @foreach($leave_codes ?? [] as $leave)
        <th>{{ $leave }} ({{$config_leave[$leave] ?? ''}})</th>
        <th>BL</th>
    @endforeach
</tr>
